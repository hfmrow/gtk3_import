...
// tagProp: used to parse tag information from chroma
type tagProp struct {
	Name  string
	Type  string
	Value interface{}
...
// tagDef: used to parse tag information from chroma
type tagDef struct {
	Name, Prio string
	Props      []tagProp
}
...
func (c *ChromaHighlight) buildTag(def tagDef) (tag *gtk.TextTag, err error) {
	...
	if tag = c.TextTagList[def.Name]; tag == nil {
		...
		switch c.srcBuff {
		case nil:
			c.TextTagList[def.Name] = c.txtBuff.CreateTag(def.Name, tagProp)
		default:
			c.TextTagList[def.Name] = c.srcBuff.CreateTag(def.Name, tagProp)
		}
	}
	return
}
