...
type ChromaHighlight struct {
	...
	txtBuff *gtk.TextBuffer
	srcBuff *source.SourceBuffer
...
// collect the visual information, the second pass compile them
...
// result in TextView/SourceView, or the Output variable' struct.
...
// using an old 4*core celeron 2.2ghz.
...
func (c *ChromaHighlight) ToTextBuff() (err error) {
	...
	case "gtkTextBuffer":
		...
		switch c.srcBuff {
		case nil:
			tags := c.txtBuff.RegisterDeserializeTagset("")
			_, err = c.txtBuff.Deserialize(c.txtBuff, tags, c.txtBuff.GetStartIter(), c.Output)

		default:
			// TODO check for DeserializeGtkSourceBuffer
			tags := c.srcBuff.RegisterDeserializeTagset("")
			_, err = c.srcBuff.Deserialize(c.srcBuff, tags, c.srcBuff.GetStartIter(), c.Output)
		}
	...
	case "pango":
		...
		switch c.srcBuff {
		case nil:
			c.txtBuff.InsertMarkup(c.txtBuff.GetStartIter(), string(c.Output))
		default:
			c.srcBuff.InsertMarkup(c.srcBuff.GetStartIter(), string(c.Output))
		}
	}
	return
}
