...
func (c *ChromaHighlight) gtkDirectToTextBufferFormatter(w io.Writer, style *chroma.Style, it chroma.Iterator) (err error) {
	...
	for tkn := it(); tkn != chroma.EOF; tkn = it() {
		...
		if !entry.IsZero() {
			...
			switch c.srcBuff {
			case nil:
				c.txtBuff.InsertWithTag(c.txtBuff.GetIterAtMark(c.txtBuff.GetInsert()), tkn.Value, tag)
			default:
				c.srcBuff.InsertWithTag(c.srcBuff.GetIterAtMark(c.srcBuff.GetInsert()), tkn.Value, tag)
			}
		} else {
			// No highlight, just insert normal text
			switch c.srcBuff {
			case nil:
				c.txtBuff.Insert(c.txtBuff.GetIterAtMark(c.txtBuff.GetInsert()), tkn.Value)
			default:
				c.srcBuff.Insert(c.srcBuff.GetIterAtMark(c.srcBuff.GetInsert()), tkn.Value)
			}
