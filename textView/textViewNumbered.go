...
import (
	...
	gitvtt "github.com/hfmrow/gtk3_import/textView/textTag"
...
func SourceViewStructNew(container gtk.Container) (svs *SourceViewStruct, err error) {
	...
	svs.container = container
	if svs.SourceView, err = source.SourceViewNew(); err == nil {
		if svs.SourceBuffer, err = svs.SourceView.GetBuffer(); err == nil {
			if svs.scrolledWin, err = gtk.ScrolledWindowNew(nil, nil); err == nil {
				svs.scrolledWin.SetProperty("visible", true)
				svs.scrolledWin.SetProperty("can-focus", true)
				svs.scrolledWin.SetProperty("hexpand", true)
				svs.scrolledWin.SetProperty("vexpand", true)
				svs.scrolledWin.SetProperty("overlay-scrolling", true)
				svs.scrolledWin.SetShadowType(gtk.SHADOW_NONE)
				svs.scrolledWin.SetPolicy(gtk.POLICY_AUTOMATIC, gtk.POLICY_ALWAYS)

				svs.scrolledWin.Add(svs.SourceView)
				svs.container.Add(svs.scrolledWin)

				svs.SourceView.SetLeftMargin(4)
				svs.SourceView.SetShowLineNumbers(true)
...
func (svs *SourceViewStruct) UpdateCss() {
	...
	svs.SourceView.SetName("tvtxt")
	gimc.CssWdgScnLoad(`
#tvtxt {
	font: `+fmt.Sprintf("%d", svs.FontSize)+`px `+svs.FontFamily+`;
}`, &svs.SourceView.Widget)

	gimc.CssWdgScnLoad(`
border {
	background-color:`+svs.NumBgCol+`
}
...
func (svs *SourceViewStruct) ScrollToLine(line int) (iter *gtk.TextIter) {
	if line > 0 && line < svs.SourceBuffer.GetLineCount() {
		...
		glib.IdleAdd(func() {
			...
			glib.TimeoutAdd(uint(64), func() bool {
				count++
				svs.SourceView.ScrollToIter(svs.SourceBuffer.GetIterAtLine(line), 0.0, true, 0.5, 0.5)
				return count <= 5
			})
		})
	}
...
func (svs *SourceViewStruct) GetCurrentLineNb() int {
	iter := svs.SourceBuffer.GetIterAtMark(svs.SourceBuffer.GetInsert())
	if iter != svs.SourceBuffer.GetEndIter() {
		return iter.GetLine()
	}
	return -1
}
...
func (svs *SourceViewStruct) SetCursorAtLine(line int) (iter *gtk.TextIter) {
	iter = svs.SourceBuffer.GetIterAtLine(line)
	svs.SourceBuffer.PlaceCursor(iter)
	svs.SourceView.GrabFocus()
	return
}
...
func (svs *SourceViewStruct) SelectRange(startLine, endLine int) {
	if startLine <= endLine && startLine > 0 {
		...
		startIter := svs.SourceBuffer.GetIterAtLine(startLine)
		endIter := svs.SourceBuffer.GetIterAtOffset(svs.SourceBuffer.GetIterAtLine(endLine).GetOffset() - 1)
		svs.SourceBuffer.SelectRange(startIter, endIter)
	}
}

// ColorBgRange: apply colored background to lines range.
func (svs *SourceViewStruct) ColorBgRange(startLine, endLine int) {
	// gitvtt.TagRemoveIfExists(svs.SourceBuffer, svs.colorBgRangeName)
	if startLine <= endLine && startLine > 0 {
		tag := gitvtt.TagCreateIfNotExists(
			svs.SourceBuffer, svs.colorBgRangeName,
			map[string]interface{}{"background": svs.ColorBgRangeSet})

		startIter := svs.SourceBuffer.GetIterAtLine(startLine)
		endIter := svs.SourceBuffer.GetIterAtOffset(svs.SourceBuffer.GetIterAtLine(endLine).GetOffset() - 1)
		svs.SourceBuffer.ApplyTag(tag, startIter, endIter)
	}
}
...
func (svs *SourceViewStruct) GetText() (text string) {
	var err error
	if text, err = svs.SourceBuffer.GetText(svs.SourceBuffer.GetStartIter(), svs.SourceBuffer.GetEndIter(), true); err != nil {
...
func (tvn *TextViewNumbered) Init(container gtk.Container) (err error) {
	...
	tvn.SelBgCol = "#CBEBEB"
	tvn.FontSze = 12
	tvn.FontFamily = `"Noto Mono Regular", sans-serif`
			...
			if tvn.tVNum, err = gtk.TextViewNew(); err == nil {
				...
				if tvn.buffNum, err = tvn.tVNum.GetBuffer(); err == nil {
					if tvn.BuffTxt, err = tvn.TextView.GetBuffer(); err == nil {
						...
						tvn.sigHdlBufTxt = tvn.BuffTxt.Connect("changed", tvn.updateNumbers)
						tvn.doCss()
					}
				}
			}
...
func (tvn *TextViewNumbered) doCss() {
	...
	gimc.CssWdgScnLoad(`
#tvtxt {
	font: `+fmt.Sprintf("%d", tvn.FontSze)+`px `+tvn.FontFamily+`;
}`, &tvn.TextView.Widget)
